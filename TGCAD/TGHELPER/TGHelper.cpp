...
bool TGHelper::IsCurActiveDocIsTemp()
{
...
CString TGHelper::GetCurActiveDocName()
{
	CString fileName;
	TGCAD* pAddin = TGCADApp::GetTGApp()->GetTGAddin();
	if (!pAddin)
...
CString TGHelper::GetCurActiveDocFullPath()
{
	CString fullname;
	TGCAD* pAddin = TGCADApp::GetTGApp()->GetTGAddin();
	if (!pAddin)
...
DocumentTypeConstants TGHelper::GetCurActiveDocType()
{
...
bool TGHelper::CurActiveDocTypeIsAsm()
{
...
bool TGHelper::CurActiveDocTypeIsPar()
{
...
bool TGHelper::CurActiveDocTypeIsDft()
{
...
bool TGHelper::CurActiveDocTypeIsPsm()
{
...
bool TGHelper::FileNameIsAsm(const CString& fileNamePath)
{
...
bool TGHelper::FileNameIsPar(const CString& fileNamePath)
{
	TCHAR drive[MAX_PATH];
	TCHAR dir[MAX_PATH];
	TCHAR fileName[MAX_PATH];
...
bool TGHelper::FileNameIsDft(const CString& fileNamePath)
{
	TCHAR drive[MAX_PATH];
	TCHAR dir[MAX_PATH];
	TCHAR fileName[MAX_PATH];
...
bool TGHelper::FileNameIsCfg(const CString& fileNamePath)
{
	TCHAR drive[MAX_PATH];
	TCHAR dir[MAX_PATH];
	TCHAR fileName[MAX_PATH];
...
bool TGHelper::FileNameIsPsm(const CString& fileNamePath)
{
	TCHAR drive[MAX_PATH];
	TCHAR dir[MAX_PATH];
	TCHAR fileName[MAX_PATH];
...
TGFramework::SolidEdgeDocumentPtr TGHelper::GetCurActiveDoc()
{
	TGCAD* pAddin = TGCADApp::GetTGApp()->GetTGAddin();
	if (!pAddin)
		return NULL;
...
bool TGHelper::SaveAsCurActiveDocToJpg(const CString& jpgFilePath)
{
...
bool TGHelper::SaveAsActiveWindowToJpg(ApplicationPtr AppPtr, const CString& jpgFilePath, bool bFitView/* = true*/)
{
	if (!AppPtr)
		return false;
	...
	if (pActiveDoc)
	{
		...
		TGDraft::SheetWindowPtr sheetwin = AppPtr->GetActiveWindow();
		ViewPtr view = NULL;
		...
		if (win)
		{
			...
			HRESULT hr = view->SaveAsImage((_bstr_t)jpgFilePath, width, height, NULL, resolution, colorDepth, TGFramework::SeImageQualityType::seImageQualityHigh, false);
			if (hr != S_OK)
				return false;
		}
		else if (sheetwin)
		{
			if (bFitView)
				sheetwin->FitEx(TGDraft::igFitAll);

			int width = sheetwin->GetUsableWidth();
			int height = sheetwin->GetUsableHeight();

			HRESULT hr = sheetwin->SaveAsImage((_bstr_t)jpgFilePath, width, height, resolution, colorDepth, TGFramework::SeImageQualityType::seImageQualityHigh, false);
			if (hr != S_OK)
				return false;
		}
		return true;
...
bool TGHelper::OpenFile(const CString& filePath)
{
...
bool TGHelper::CloseFile(const CString& filePath)
{
...
bool TGHelper::CloseCurActiveFile()
{
...
bool TGHelper::SaveCurActiveDocToFilePath(const CString& saveFilePath)
{
...
bool TGHelper::GetAllCurOpenDocs(std::vector<CString>& getFiles)
{
	DocumentTypeConstants docType = DocumentTypeConstants::igUnknownDocument;
...
bool TGHelper::DocIsDirty(const CString& fileNamePath)
{
...
HWND TGHelper::GetCADHwnd()
{
	CWnd* pWnd = TGCADApp::GetTGApp()->GetMainWnd();
	if (pWnd)
	{
		return pWnd->m_hWnd;
	}
...
bool TGHelper::ExportCurActiveDoc(const CString& exportDocFileNamePath)
{
...
bool TGHelper::CallCadCommand(long cmdId)
{
	try
	{
		TGCAD* pAddin = TGCADApp::GetTGApp()->GetTGAddin();
		...
		ApplicationPtr AppPtr = pAddin->GetApplication();
		HRESULT hr = AppPtr->StartCommand((TGFramework::SolidEdgeCommandConstants)cmdId);
		if (hr == S_OK)
			return true;
	}
	catch (...)
