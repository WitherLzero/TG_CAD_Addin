...
LANGID GetSolidEdgeUserDefaultLangID( ApplicationPtr& pTheEdge )
{
	...
	if( pTheEdge )
	{
		try
		{
			...
			pTheEdge->GetGlobalParameter( TGFramework::seApplicationGlobalDefaultUserLangID, &vLangID );
...
STDMETHODIMP TGCAD::raw_OnConnection( IDispatch *pAppDispatch, SeConnectMode ConnectMode, AddIn* pAddIn )
{
	AFX_MANAGE_STATE( AfxGetStaticModuleState() );

	m_pApplication = pAppDispatch;
	TGCADApp::GetTGApp()->SetTGAddin(this);
	s_hMyInstance = AfxGetResourceHandle();
	HRESULT hr = S_OK;

	TGEventManagerObj::CreateInstance(&m_pCommands);
	m_pCommands->AddRef();
	...
	TGCmdManager::Instance()->InitCommandMap();
	return( hr );
}

#define NUMBER_OF_COMMANDS 2// Number of commands on the menu (V20 and earlier) or in a group (ST1 and later).

STDMETHODIMP TGCAD::raw_OnConnectToEnvironment( BSTR EnvironmentCatid, LPDISPATCH pEnvironment, VARIANT_BOOL bFirstTime )
{
	HRESULT hr = S_OK;
	...
	for (int iGUID = 0; iGUID < m_vGUID.size(); iGUID++)
	{
		if (IsEqualGUID(m_vGUID[iGUID], environmentGuid) == TRUE)
		{
		...
		if( NULL != pAddInEx )
		{
			...
			if( NULL != pAddInEx2 )
			{
				...
				VERIFY_OK(pAddInEx2->SetAddInInfoEx2(ResourceFilename, EnvironmentCatid,
					bszCategoryName1, IDB_BITMAP1, IDB_BITMAP1, IDB_BITMAP1,
					IDB_BITMAP1, NUMBER_OF_COMMANDS, saTab1Group1CommandStrings.GetSafeArrayPtr(),
			...
			else
			{
				VERIFY_OK(pAddInEx->SetAddInInfoEx(ResourceFilename, EnvironmentCatid,
					bszCategoryName1, IDB_BITMAP1, IDB_BITMAP1, IDB_BITMAP1,
					IDB_BITMAP1, NUMBER_OF_COMMANDS, saTab1Group1CommandStrings.GetSafeArrayPtr(), saTab1Group1CommandIDs.GetSafeArrayPtr()));
			}
			...
			if( bEdgeSupportsMultipleMenus )
			{
				...
				if( NULL != pAddInEx2 )
				{
					...
					VERIFY_OK(pAddInEx2->SetAddInInfoEx2(ResourceFilename, EnvironmentCatid,
						bszCategoryName1, IDB_BITMAP1, IDB_BITMAP1, IDB_BITMAP1,
						IDB_BITMAP1, NUMBER_OF_COMMANDS, saTab1Group2CommandStrings.GetSafeArrayPtr(),
					...
					VERIFY_OK(pAddInEx2->SetAddInInfoEx2(ResourceFilename, EnvironmentCatid,
						bszCategoryName1, IDB_BITMAP1, IDB_BITMAP1, -1,
					...
					VERIFY_OK(pAddInEx2->SetAddInInfoEx2(ResourceFilename, EnvironmentCatid,
						bszCategoryName2, IDB_BITMAP1, IDB_BITMAP1, IDB_BITMAP1,
						IDB_BITMAP1, NUMBER_OF_COMMANDS, saTab2Group1CommandStrings.GetSafeArrayPtr(),
...
STDMETHODIMP TGCAD::raw_OnDisconnection( SeDisconnectMode DisconnectMode )
{
	AFX_MANAGE_STATE( AfxGetStaticModuleState() );
	...
	// ���CmdManager
	TGCmdManager::Instance()->Destroy();

  return hr;
}

STDMETHODIMP TGCAD::InterfaceSupportsErrorInfo(REFIID riid)
{
...
HRESULT TGCAD::CreateRibbon(GUID environmentGuid, EnvironmentPtr pEnvironment, VARIANT_BOOL bFirstTime)
{
	HRESULT hr = S_OK;
	...
	try
	{
		...
		::StringFromGUID2(CLSID_TGAddIn, szGuid, GUID_STRING_LENGTH);
		...
		ISEAddInEx2Ptr pAddInEx2 = m_pCommands->GetAddIn();
	//	ISEAddInExPtr pAddInEx = m_pCommands->GetAddIn();
		TGUIManager& TGUI = TGUIManager::getInstance();
		TGUI.createUserUI(pAddInEx2, environmentGuid, pEnvironment, strCommandPrefix, bstrResourceFilename);
	}
	catch (_com_error &e)
	{
		hr = e.Error();
...
ApplicationPtr TGCAD::GetApplication()
{
